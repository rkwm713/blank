...
def determine_proposed_heights(pole_data, connection_heights):
    """Determine proposed heights for midspan based on move indicators."""
    proposed_heights = {}
...
def identify_ref_subgroups(pole_data):
    ...
    for connection in pole_data.get('connections', []):
        ...
        if is_ref:
            ...
            ref_groups.append({
                'connection_id': connection.get('connection_id'),
                'label': f"Red ({direction}) to {description}",
...
def create_make_ready_excel(processed_pole_data, output_filepath):
    ...
    for pole in processed_pole_data:
        ...
        attachers_count = len(pole.get('attachers', []))
        ...
        total_rows = max(1, attachers_count) + 2 + ref_group_rows
        ...
        for attacher in pole.get('attachers', []):
            # Attacher data - safely extract string values
            ws.cell(row=attacher_row, column=12).value = extract_string_value(attacher.get('description'))
            ws.cell(row=attacher_row, column=13).value = extract_string_value(attacher.get('existing_height'))
            ws.cell(row=attacher_row, column=14).value = extract_string_value(attacher.get('proposed_height'))
            ws.cell(row=attacher_row, column=15).value = extract_string_value(attacher.get('midspan_proposed'))
        ...
        # Existing Mid-Span Data Row (if available in data structure)
        midspan_row += 1
        lowest_com_cell = ws.cell(row=midspan_row, column=10)
        ...
        lowest_cps_cell = ws.cell(row=midspan_row, column=11)
        ...
        proposed_midspan_cell.value = extract_string_value(pole.get('midspan_proposed'))
